...
const JumboAuthProvider = ({children, ...restProps}) => {
    ...
    const setAuthToken = React.useCallback(async (token) => {
        ...
        try {
            ...
            if (authUser) {
                dispatch({ type: AUTH_ACCESS, payload: authUser?.result?.roleData });
                setAuthOptions({
                    type: "set-auth-data",
                    payload: {authToken: token, authUser: authUser?.result, isLoading: false}
                });
                localStorage.setItem("access_token", JSON.stringify(authUser?.result?.roleData));
                return;
            }
            setLogout(true);
